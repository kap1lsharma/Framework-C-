...
Feature: OL_AR_DonotArchival_Loyality_Lite Verifing Complete flow of Do not Archival of a Player through Loyality Lite

#***********************************************************************
#** Feature File                    : OL_AR_DonotArchival_Loyality_Lite
...
#*****************************************************************/
Scenario Outline: 001 Do not Archival of Player with Permission through Loyality Lite (TC ID - 475336)
	...
	Given User enters text "SC_DONOTARCHIVE" in "OLW_USER_GROUP_PERMISSION.txtSearchPermission"
	...
	Then User "checks" element "OLW_USER_GROUP_PERMISSION.chkPermissionNameCheckbox" with parameters
		| Key            | Value           |
		| permissionname | SC_DONOTARCHIVE |
	...
	Given User clicks on "LLPlayerArchival.btn_DoNotArchive"
	...
	When User performs string operation "stringEquals" of provided value "<Donot_ArchivalMessage>" with "<$Message$>"
	When User waits "2" seconds
	Given User clicks on "LLPlayerArchival.btn_Close"
	#User verifes AR button on player
	When User gets attribute "Name" value of "LLPlayerArchival.btn_NA_Indicator" element to save with key "NA_Button_Text"
	When User performs string operation "stringEquals" of provided value "<NA_Button>" with "<$NA_Button_Text$>"
	#User count in UserTransactionLog table
	When User runs query "SELECT Count(1) FROM UC_X_UserTransactionLog WHERE PlayerID= <$REAL_PLAYER_ID$> AND userid= '<#Halo_username#>' And UsertransactionTypeId in(229) AND PropertyID=<$PropertyID$> ORDER BY 1 DESC" with connection "<#Halo_db_connectionstring#>" and save with key "Row_Count"
	When User performs string operation "stringEquals" of provided value "<$Row_Count$>" with "1"
	Given User closes application

	Examples:
		| archive_reason           | Comment_For_Archival      | Donot_ArchivalMessage                        | NA_Button |
		| Instructed by Management | Comment Line For Archival | Player Successfully marked for Non Archival. | NA        |

Scenario Outline: 002 Do not Archival of Player without Permission through Loyality Lite (TC ID - 475336)
	#User login into Web-APP
	Given User login halo web app with "<#Halo_APPURL#>:<#Halo_username#>:<#Halo_password#>:<#Halo_domain#>" information
	Given User enters text "Group Permission" in "MAIN_NAV_BOX.MenuSearch"
	When User waits "1" seconds
	...
	Given User clicks on "LLPlayerArchival.btn_Player_Archival_Status"
	When User waits "2" seconds
	When User gets attribute "enabled" value of "LLPlayerArchival.btn_DoNotArchive" element to save with key "Locator_Enable_State"
...
Scenario: 003 Renabling Do not Archival Player Permission from Web-APP
	...
	Given User enters text "SC_DONOTARCHIVE" in "USER_GROUP_PERMISSIONS.PERMISSION_SEARCH_TEXTBOX"
